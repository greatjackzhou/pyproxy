...
class Header:
    ...
    def get_host_info(self):
        ...
        if self._host is None:
            ...
            line = self.header_list[0].decode(self._encoding)
            if method == b"CONNECT":
                host = line.split(' ')[1]
                if ':' in host:
                ...
                for i in self.header_list:
                    if i.startswith(b"Host:"):
                        ...
                        if len(host) < 2:
                            continue
                        host = host[1].decode(self._encoding)
                        break
                else:
                    host = line.split('/')[2]
                if ':' in host:
    ...
    def __repr__(self):
        return str(self._header.decode(self._encoding))
...
if __name__ == '__main__':
    IP = "172.29.1.81" #192.168.1.204#172.29.1.81
    PORT = 8888 
    server(IP, PORT)