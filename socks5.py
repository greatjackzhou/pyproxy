...
class DYProxy(Tcp):
    ...
    def handle(self):
        ...
        if address_type == 1:       # 0x01表示IPv4地址，DST.ADDR为4个字节
            # 转换IPV4地址字符串（xxx.xxx.xxx.xxx）成为32位打包的二进制格式（长度为4个字节的二进制字符串）
            address = socket.inet_ntoa(self.connection.recv(4))
        elif address_type == 3:     # 0x03 表示域名，DST.ADDR是一个可变长度的域名
...
if __name__ == '__main__':
    ...
    IP = "172.29.1.81" #172.29.1.81#192.168.1.204
    PORT = 8889
    Server = ThreadingTCPServer((IP, PORT), DYProxy)
    print("**********************************************************")
    print("************************* DYPROXY ************************")
    print("*************************   1.0   ************************")
    print("********************  IP:%s  ******************" % IP)
